...
public class DirectoryMapper 
{
    ...
    */
    private void mapRecursive(String aPath)
    {
        ...
        for (File f : fileList) if (m_Logger != null) m_Logger.log(f.getPath().replace(m_DropboxPublicDirectoryRoot,m_DropboxPublicRootURL).replace("\\","/"));            
        
        if (m_Logger != null) m_Logger.log("");
        
        for (File f : directoryList) mapRecursive(f.getPath()/*.getAbsolutePath()*/);            
        ...
        try 
        {
            ...
            for ( File f : directoryList)
            {
                ...
                if (directoryList[directoryList.length-1] != f) bw.write(",");   
		...
		bw.newLine();                
            ...
            for ( File f : fileList)
            {
                ...
                if (fileList[fileList.length-1] != f) bw.write(",");
        ...
        {
            if (m_Logger != null) m_Logger.log(ex.getMessage());
