...
{        
    ...
    public static void main(String[] args) 
    {
        ...
        if (!m_NoGUI) guiManager = new GUIManager();
        
        if (guiManager != null) guiManager.log(
            "DropBoxWebInterface\n" +
        ...
        if (loadSettings())
        {
            ...
            if (m_NoGUI) System.exit(0);
            
            else guiManager.log("*********\nMapping is complete\n*********");   
        }
        else if (guiManager != null) guiManager.log("Settings could not be loaded from Settings.json. Please verify json contents and rerun the program.");            
    }
    ...
    private static boolean loadSettings()
    {
        ...
        if (!file.exists())
        {
            if (guiManager != null) guiManager.log("Settings.json did not exist. Creating the file. Please fill out Settings.json with the needed information");
            ...
            {
                if (guiManager != null) guiManager.log(ex.getMessage());
    ...
    {
        for (String s: args) if (s.contentEquals("noGUI")) m_NoGUI = true;
    }
